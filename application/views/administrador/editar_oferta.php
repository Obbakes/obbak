...
<script>
	window.imagenVacia = '<?php echo base_url() . $oferta->imagen . '?num=' . mt_rand(1, 1000000000); ?>';
		...
		if(Archivos.length>0){
			...
			Lector.onloadend = function(e){
				...
				if(tipo!=='image/jpeg' && tipo!=='image/png' && tipo!=='image/gif'){
					jQuery('#vistaPrevia').css('background-image', 'url(\'' + window.imagenVacia + '\')');
					jQuery('#vistaPrevia').css('display', 'none');
					jQuery('#vistaPreviaLista').css('background-image', 'url(\'' + window.imagenVacia + '\')');
					jQuery('#vistaPreviaDetalles').css('background-image', 'url(\'' + window.imagenVacia + '\')');
					$('#imagen').val('');
					mostrarFichero();
	...
	jQuery(document).ready(function(){
		...
		obtenerDescripcionMedio();
		getMedios(<?php echo (!empty($validado)) ? ((set_value('id_medio') != '') ? set_value('id_medio') : 0) : $oferta->id_medio; ?>);
	});
...
<div style="display: table; margin: 20px auto; width: 50%; padding: 0;">
	...
	<div style="display: table-cell; width: 50%; text-align: center;">
		<input type="button" class="boton-pestanya-activo" value="Inscripciones" style="width: 90%;"
			onclick="document.location.href='<?php echo base_url() . 'administrador/inscripcionesOferta/' . $oferta->id_oferta; ?>';">
	</div>
</div>
...
<div style="display: table; margin: auto;max-width:960px;">
	<div style="display: table-cell; width: 50%; margin-bottom: 5px; text-align: left;">
		<?php echo form_open_multipart("administrador/editarOferta/" . $oferta->id_oferta . "/1"); ?>
			<p>
				...
				<textarea id="titulo" class="contacto_form_input <?php echo (form_error('titulo') != '') ? 'input_error' : ''; ?>" maxlength="40"
					...
					onchange="actualizarCampoVista('titulo', 'titulo_vista_previa', ''); actualizarCampoVista('titulo', 'titulo_vista_detalles', '');"><?php echo (!empty($validado)) ? set_value('titulo') : $oferta->titulo; ?></textarea>
				...
				<select id="id_tipo_oferta" name="id_tipo_oferta" class="contacto_form_input" style="width: 70%;">
                                    ...
                                    ?>
                                                <option value="<?php echo $tipo_oferta->id_tipo_oferta ?>" <?php echo (!empty($validado)) ? set_select('id_tipo_oferta', $tipo_oferta->id_tipo_oferta) : (($oferta->id_tipo_oferta == $tipo_oferta->id_tipo_oferta) ? 'selected="selected"' : ''); ?>>
...
?>
					<option value="<?php echo $tipo_medio->id_tipo ?>" <?php echo (!empty($validado)) ? set_select('id_tipo_medio', $tipo_medio->id_tipo) : (($oferta->id_tipo_medio == $tipo_medio->id_tipo) ? 'selected="selected"' : ''); ?>>
...
?>
			...
			<p>
				...
				<select id="provincia" name="provincia" class="contacto_form_input" style="width: 70%;">
                                    ...
                                    ?>
                                                <option value="<?php echo $provincia->id_provincia ?>" <?php echo (!empty($validado)) ? set_select('provincia', $provincia->id_provincia) : (($oferta->id_provincia == $provincia->id_provincia) ? 'selected="selected"' : ''); ?>>
			...
			<p>
				...
				<select id="sector" name="sector" class="contacto_form_input" style="width: 70%;">
                                    ...
                                    ?>
                                                <option value="<?php echo $sector->id_sector ?>" <?php echo (!empty($validado)) ? set_select('sector', $sector->id_sector) : (($oferta->id_sector == $sector->id_sector) ? 'selected="selected"' : ''); ?>>
			...
			<p>
				...
				<textarea id="descripcion" class="contacto_form_input <?php echo (form_error('descripcion') != '') ? 'input_error' : ''; ?>"
					...
					onchange="actualizarCampoVista('descripcion', 'descripcion_vista_previa', '');"><?php echo (!empty($validado)) ? set_value('descripcion') : $oferta->descripcion; ?></textarea>
			...
			<p>
				...
				<textarea id="detalle" class="ckeditor contacto_form_input <?php echo (form_error('detalle') != '') ? 'input_error' : ''; ?>" name="detalle"
					...
					onchange="actualizarCampoVista('detalle', 'detalles_vista_previa_detalles', '');"><?php echo (!empty($validado)) ? set_value('detalle') : $oferta->detalle; ?></textarea>
			...
			<p>
				...
				<textarea id="condiciones" class="ckeditor contacto_form_input <?php echo (form_error('condiciones') != '') ? 'input_error' : ''; ?>" name="condiciones"
					...
					onchange="actualizarCampoVista('condiciones', 'condiciones_vista_previa_detalles', '');"><?php echo (!empty($validado)) ? set_value('condiciones') : $oferta->condiciones; ?></textarea>
			...
			<p>
				...
				<input type="text" id="duracion_camp" class="contacto_form_input <?php echo (form_error('duracion_camp') != '') ? 'input_error' : ''; ?>"
					name="duracion_camp" value="<?php echo (!empty($validado)) ? set_value('duracion_camp') : $oferta->duracion_camp; ?>" style="width: 335px; height: 40px;"
			...
			<p>
				...
				<input type="text" id="precio_anterior" name="precio_anterior"
					...
					value="<?php echo (!empty($validado)) ? set_value('precio_anterior') : $oferta->precio_anterior; ?>"
			...
			<p>
				...
				<input type="text" id="precio_oferta" name="precio_oferta"
					...
					value="<?php echo (!empty($validado)) ? set_value('precio_oferta') : $oferta->precio_oferta; ?>"
					style="background-position: right center; padding-right: 36px; background-image: url(<?php echo base_url(); ?>images/ico_euro.png);
					background-repeat: no-repeat; width: 335px; height: 40px; text-align: right;"
			...
			<p>
				...
				<input type="text" id="descuento" name="descuento" disabled="disabled"
					...
					value="<?php echo (!empty($validado)) ? set_value('descuento') : $oferta->descuento; ?>"
			...
			<p>
				...
				<input type="text" id="coste_real" name="coste_real" class="contacto_form_input <?php echo (form_error('coste_real') != '') ? 'input_error' : ''; ?>"
					value="<?php echo (!empty($validado)) ? set_value('coste_real') : $oferta->coste_real; ?>"
					style="background-position: right center; padding-right: 36px; background-image: url(<?php echo base_url(); ?>images/ico_euro.png);
						background-repeat: no-repeat; width: 335px; height: 40px; text-align: right;" />
				<?php echo form_error('coste_real'); ?>
			</p>
			<p>
				<label for="fecha_insercion" style="display: block; width: 100%;">
					Fecha Inserción
				</label>
				<input type="text" id="fecha_insercion" name="fecha_insercion"
					class="datepicker_time contacto_form_input <?php echo (form_error('fecha_insercion') != '') ? 'input_error' : ''; ?>"
					<?php echo (!empty($validado)) ? ((set_value('fecha_insercion') != '') ? 'disabled="disabled"' : '') : ((empty($oferta->fecha_insercion)) ? 'disabled="disabled"' : ''); ?>
					value="<?php echo (!empty($validado)) ? set_value('fecha_insercion') : $oferta->fecha_insercion; ?>" style="width: 335px; height: 40px;" readonly />
			</p>
			<p>
				...
				<input type="text" id="fecha_fin_pub" name="fecha_fin_pub"
					...
					<?php echo (!empty($validado)) ? ((set_value('fecha_fin_pub') != '') ? 'disabled="disabled"' : '') : ((empty($oferta->fecha_fin_pub)) ? 'disabled="disabled"' : ''); ?>
					value="<?php echo (!empty($validado)) ? set_value('fecha_fin_pub') : $oferta->fecha_fin_pub; ?>" style="width: 335px; height: 40px;" readonly
			...
			<p>
				...
				<input type="checkbox" id="fecha_fin_pub_indef" name="fecha_fin_pub_indef" value="1"
					...
					<?php echo (!empty($validado)) ? set_checkbox('fecha_fin_pub_indef', 1) : ((empty($oferta->fecha_fin_pub)) ? 'checked="checked"' : ''); ?> style="width: auto;"/>
				...
				<input type="text" id="det_duracion_camp" class="contacto_form_input <?php echo (form_error('det_duracion_camp') != '') ? 'input_error' : ''; ?>"
					name="det_duracion_camp" value="<?php echo (!empty($validado)) ? set_value('det_duracion_camp') : ((!empty($oferta->detalle_fin_camp)) ? $oferta->detalle_fin_camp : ''); ?>" style="display:none;width: 335px; height: 40px"
                                        onchange="actualizarFechaFinPublicacion();"/>
			...
			<p>
				...
				<input type="text" id="link" name="link"
					...
					value="<?php echo (!empty($validado)) ? set_value('link') : $oferta->link; ?>" style="width: 335px; height: 40px;" />
			...
			<p>
				...
				<input type="checkbox" id="publicada" name="publicada" value="1" style="width: auto;"
					<?php echo (!empty($validado)) ? set_checkbox('publicada', 1) : ((empty($oferta->publicada)) ? '' : 'checked="checked"'); ?> />
			...
			<p>
				...
				<input type="text" id="galeria_img" name="galeria_img"
					...
					value="<?php echo (!empty($validado)) ? set_value('galeria_img') : $oferta->galeria_img; ?>" style="width: 335px; height: 40px;" />
			...
			<table cellspacing="0" cellpadding="0"
				...
				<tr>
					<td id="vistaPreviaLista" rowspan="10" width="40%" style="background-position: center center; background-repeat: no-repeat;
						background-size: contain; background-image: url('<?php echo base_url() . $oferta->imagen . '?num=' . mt_rand(1, 1000000000); ?>'); margin-left: 1px;
						margin-top: 1px;">
					</td>
				...
				<tr>
					<td colspan="2" width="30%" style="border-right: 1px solid lightgray; padding: 3.5px 7.8px 3.5px 16.3px; text-align: center;" bgcolor="white">
				...
				<tr>
					...
					<td colspan="2" width="30%" style="text-align: right; padding: 3.5px 16.3px 7.1px 0;" bgcolor="white">
						<div style="margin-left: auto; padding: 3.5px 0; border-radius: 4px; background: #ff8600; text-align: center; color: white;
							font-weight: bold; font-size: 13.4px; width: 106px;" >
							VER DETALLES
						</div>
					</td>
				</tr>
			...
			<table style="border: 1px solid lightgray; width: 100%; border-spacing: 0; font-family: calibri; color: #333; margin-bottom: 10px;">
				...
				<tr>
					<td id="vistaPreviaDetalles" width="50%" id="vistaPreviaDetalles" style="background-position: center center;
							background-repeat: no-repeat; background-size: contain; background-image: url('<?php echo base_url() . $oferta->imagen . '?num=' . mt_rand(1, 1000000000); ?>');
