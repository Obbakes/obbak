...
<div>
	<div style="position: relative; width: 360px; display:inline-table; margin-bottom: 5px; text-align: left; right: 20px;">
		...
		<form accept-charset="utf-8" method="post" action="<?php echo base_url() . 'administrador/permisosMedio/' . $medio->id_medio . '/1'; ?>" >
...
?>
				<tr <?php if ($i%2==0) echo 'class="item_fav"';?>>
					...
					<td>
						<?php echo $permiso->email ?>
					</td>
                                        <td>
						<?php echo $permiso->cif ?>
					</td>
					<td>
						<?php echo $permiso->provincia ?>
					</td>
					<td style="vertical-align: middle;" class="select_permiso">
                                                
						<select name="permiso" class="contacto_form_input changeGroup" data-id="<?php echo $permiso->id_cliente; ?>" autocomplete="off"
...
?>
			...
			<div class="paginacion">
				<?php echo $paginacion; ?>&nbsp;&nbsp;<a href="<?php echo base_url().'administrador/permisosMedio/'.$medio->id_medio.'/2/1/400'?>">Mostrar Todos</a>
			</div>
		</div>
		<form id="form_permisos" action="<?php echo base_url() .'administrador/cambiarPermisosMedio/' . $medio->id_medio; ?>"
